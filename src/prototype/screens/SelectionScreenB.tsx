...
import styles from './SelectionScreenB.module.css';
...
interface SelectionScreenBProps { viewport?: 'mobile' | 'desktop'; }

export function SelectionScreenB({ viewport = 'desktop' }: SelectionScreenBProps) {
              ...
              return (
                <div key={service.id} className={[styles.card, isSelected ? styles.cardSelected : '', isDisabled ? styles.cardDisabled : ''].filter(Boolean).join(' ')}>
                  <button className={styles.cardHeader} onClick={() => !isDisabled && toggleService(service.id)} disabled={isDisabled}>
                    <div className={[styles.cardIcon, isSelected ? styles.cardIconSelected : ''].filter(Boolean).join(' ')}><service.Icon size={20} /></div>
