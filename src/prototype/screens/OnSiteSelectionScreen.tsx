...
const EVENTS: EventConfig[] = [
  ...
  },
  {
    ...
    services: [
      ...
      { id: 'med-profunda', name: 'Meditação Profunda', duration: 30, description: 'Sessão guiada de atenção plena',         Icon: Wind,    globallyExhausted: true },
    ],
  },
];
...
function generateSlots(serviceId: string) {
  ...
  return base.map((time, i) => ({ time, available: (13 + i + seed) % 3 !== 0 }));
}
...
export function OnSiteSelectionScreen({ viewport = 'desktop', onNavigate }: OnSiteSelectionScreenProps) {
  ...
  const canProceed = selectedCount > 0 && event !== null && [...selectedIds].every(isServiceComplete);
                ...
                return (
                  ...
                  >
                    <button
                      ...
                      onClick={() => isCollapsed ? setExpandedId(svc.id) : toggleService(svc.id)}
                      disabled={isDisabled}
                    >
                      <div className={[
                        styles.cardIcon,
                        isSelected ? styles.cardIconSelected : '',
                        isSelected && svc.globallyExhausted ? styles.cardIconWaitlist : '',
                      ].filter(Boolean).join(' ')}>
                        <svc.Icon size={18} />
                      </div>
                      ...
                      <div className={styles.cardInfo}>
                        ...
                        {isCollapsed && isComplete ? (
                          <span className={styles.cardSummary}>
                            ...
                            {svc.globallyExhausted ? 'Lista de espera' : `Hoje · ${sch?.time}`}
                          </span>
                        ) : (
                        ...
                        {isExpanded && isComplete && (
                          <span className={styles.cardComplete}>
                            <CheckCircle2 size={12} />
                            {svc.globallyExhausted ? 'Na lista de espera' : `Hoje · ${sch?.time}`}
                          </span>
                        )}
                      </div>
                    ...
                    {isSelected && svc.globallyExhausted && (
                      <div className={styles.cardBody}>
                        <Feedback type="warning" title="Horários esgotados" message="Você foi adicionado à lista de espera. Entraremos em contato se uma vaga abrir." />
                      </div>
                    )}
